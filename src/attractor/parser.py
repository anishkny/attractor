...
class DotParser:
    ...
    def parse_edge(self):
        ...
        self.skip_whitespace()
        attrs = self.parse_attr_block() if self.peek_char() == "[" else {}
        ...
        for i in range(len(chain) - 1):
            ...
            if chain[i + 1] not in self.nodes:
                self.nodes[chain[i + 1]] = Node(
                    id=chain[i + 1], attrs={**self.node_defaults}
                )

            self.edges.append(
                Edge(from_node=chain[i], to_node=chain[i + 1], attrs=edge_attrs.copy())
            )
