...
from .models import Graph, Node, Context, Outcome, StageStatus, Checkpoint
from .handlers import HandlerRegistry, CodergenHandler
...
class PipelineEngine:
    ...
    def run(self, context: Optional[Context] = None) -> Outcome:
        ...
        while True:
            ...
            outcome = self._execute_with_retry(node, context, retry_policy, node_retries)
            ...
            self._save_checkpoint(context, current_node.id, completed_nodes, node_retries)
    ...
    def _save_checkpoint(self, context: Context, current_node: str, completed_nodes: List[str], node_retries: dict):
        """Save a checkpoint."""
